...
const STATUS_STEPS = [
  {
    ...
    icon: Package,
    desc: "Your delivery has been scheduled",
  },
  {
    key: "picked_up",
    label: "Picked Up",
    icon: Truck,
    desc: "Package collected from origin",
  },
  {
    key: "in_progress",
    label: "In Transit",
...
function TrackDelivery() {
  ...
  const fetchTracking = useCallback(async (t: string) => {
    ...
    const { data, error: err } = (await supabase.rpc("get_tracking_info", {
      p_token: t.trim().toUpperCase(),
    })) as {
  ...
  const currentStep = tracking
    ? getStatusIdx(tracking.status ?? "assigned")
    : 0;
      ...
      <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
          ...
          <div className="flex gap-2 max-w-md mx-auto mt-4">
            <div className="relative flex-1">
              ...
              <Input
                ...
                onKeyDown={(e) =>
                  e.key === "Enter" && fetchTracking(searchInput)
                }
        ...
        {tracking && (
          <>
            ...
            <Card className="border-0 bg-white/5 backdrop-blur-sm overflow-hidden">
              <div
                className={`h-1.5 ${isDelivered ? "bg-green-500" : "bg-gradient-to-r from-violet-500 to-indigo-500"}`}
              />
                    ...
                    return (
                      <div key={step.key} className="flex items-center flex-1">
                        <div className="flex flex-col items-center gap-1.5">
                          ...
                          >
                            <Icon
                              className={`h-4 w-4 ${isComplete ? "text-white" : "text-white/30"}`}
                            />
                          </div>
                          <span
                            className={`text-[10px] ${isComplete ? "text-white/80" : "text-white/30"} text-center leading-tight`}
                          >
                            {step.label}
                          </span>
                        </div>
                        {i < STATUS_STEPS.length - 1 && (
                          <div
                            className={`flex-1 h-0.5 mx-2 mb-4 ${i < currentStep ? "bg-green-500" : "bg-white/10"}`}
                          />
                        )}
                      </div>
                    );
            ...
            <div className="grid grid-cols-2 gap-3">
                  ...
                  <CardContent className="py-4 flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-2xl font-bold text-white">
                        {formatTime(tracking.estimated_arrival)}
                      </p>
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card className="border-0 bg-white/5">
                <CardContent className="py-3">
                  ...
                  <p className="text-sm font-semibold text-white mt-1">
                    {tracking.packages ?? 1} items
                  </p>
                  <p className="text-[10px] text-white/30 mt-1">
                    {tracking.service_type ?? "Standard"}
                  </p>
                </CardContent>
              </Card>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-green-400/60">
                          ...
                          {tracking.pod_signer_name &&
                            ` -- Signed by ${tracking.pod_signer_name}`}
                        </p>
                      </div>
                    </div>
                    {/* POD Photo + Signature */}
                  ...
                  <CardContent className="py-4">
                    ...
                    <div className="relative h-48 bg-gradient-to-br from-slate-800 to-slate-900 rounded-xl overflow-hidden">
                      ...
                      <div className="absolute inset-0 flex items-center justify-center">
                        <div
                          className="absolute h-32 w-32 rounded-full border border-violet-500/20 animate-ping"
                      ...
                      {/* Speed indicator */}
                      {tracking.driver_speed !== null &&
                        tracking.driver_speed !== undefined && (
        ...
        {!tracking && !error && !loading && !urlToken && (
          <div className="text-center pt-12 space-y-4">
            ...
            <p className="text-white/20 text-xs">Format: ANK-XXXXXXXX</p>
          </div>
        )}
      </div>
