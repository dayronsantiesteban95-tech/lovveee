...
function DriverPortal() {
  ...
  const fetchLoads = useCallback(async () => {
    ...
    const { data } = await supabase
      .from("daily_loads")
      .select(
        "id, reference_number, client_name, pickup_address, delivery_address, status, packages, service_type, start_time, end_time, wait_time_minutes, comments",
      )
    ...
    if (!onDuty) {
      ...
      if (!granted) {
        toast({
          ...
          description: "Please allow location access to go on duty.",
          variant: "destructive",
        });
        return;
      }
      ...
      toast({
        title: "On Duty",
        description: "GPS tracking started. Drive safe!",
      });
    } else {
      // Go off duty
      if (shiftId) {
        try {
          ...
          captureFleetError(driver.id, err, {
            operation: "end_shift",
            shiftId,
          });
          return;
      ...
      toast({
        title: "Off Duty",
        description: "GPS tracking stopped. Good job today!",
      });
    }
  };
  ...
  const advanceStatus = async (load: DriverLoad) => {
    ...
    try {
      ...
      setStatusNote("");
      toast.success(
        `${getStatusInfo(nextStatus).label} - ${load.client_name ?? "Load updated"}`,
      );
      fetchLoads();
    } catch (err) {
      toast.error("Failed to update load status");
      captureLoadError(load.id, err, {
      ...
      <div className="min-h-screen bg-background flex items-center justify-center p-6">
        <Card className="max-w-sm w-full border-0 shadow-lg">
          <CardContent className="pt-8 pb-6 text-center space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              Your account isn't linked to a driver profile. Ask your dispatcher
              to assign you.
            </p>
          </CardContent>
        </Card>
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      {/* -- Header -- */}
      <div
        className={`px-4 py-4 ${onDuty ? "bg-green-600" : "bg-slate-800"} text-white transition-colors duration-500`}
      >
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs opacity-80 flex items-center gap-1">
              ...
              {driver.hub.charAt(0).toUpperCase() + driver.hub.slice(1)} Hub
            </p>
          </div>
          <div className="text-right">
            <Badge
              className={`${onDuty ? "bg-white/20 text-white" : "bg-white/10 text-white/60"} text-xs`}
            >
              {onDuty ? "On Duty" : "Off Duty"}
            </Badge>
            {gps.tracking && (
              <p className="text-[10px] opacity-60 mt-1 flex items-center gap-1 justify-end">
                <Radio className="h-2.5 w-2.5 animate-pulse" /> GPS active ?{" "}
                {gps.pingCount} pings
              </p>
            )}
          </div>
      ...
      {onDuty && (
        <div className="px-4 py-2 bg-muted/50 border-b flex items-center gap-3 text-xs">
          ...
          {gps.position?.speed !== null &&
            gps.position?.speed !== undefined && (
              <span className="text-muted-foreground">
                {Math.round(gps.position.speed * 2.237)} mph
              </span>
            )}
          {gps.lastPingAt && (
            <span className="ml-auto text-muted-foreground">
              Last ping: {timeAgo(gps.lastPingAt)}
            </span>
          )}
        </div>
      )}
      ...
      <div className="px-4 py-4 space-y-4">
        ...
        <Button
          onClick={toggleDuty}
          className={`w-full h-14 text-base font-bold rounded-2xl transition-all shadow-lg ${
            onDuty
            ...
            <CardContent className="py-3 px-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Allow location access in your browser settings to use GPS
                  tracking.
                </p>
              </div>
            </CardContent>
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            {load.reference_number
                              ? `#${load.reference_number}`
                              : ""}{" "}
                            ? {load.packages} pkg ? {load.service_type}
                          </p>
                        </div>
                        <Badge
                          className={`${statusInfo.color} text-white text-[10px]`}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => {
                            toast({
                              title: "Call client",
                              description:
                                "Client phone not yet configured for this load.",
                            });
                          }}
                        >
                            ...
                            input.onchange = async (e) => {
                              ...
                              if (uploadErr) {
                                ...
                                try {
                                  ...
                                  toast.error(
                                    "POD uploaded but failed to confirm",
                                  );
                                  captureLoadError(load.id, podErr, {
                                    operation: "confirm_pod",
                                  });
                                }
                              }
                            };
