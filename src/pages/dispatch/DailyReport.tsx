...
export default function DailyReport({
  ...
  const dailyReport = useMemo(() => {
    ...
    const totalCosts = dayLoads.reduce(
      (s, l) => s + Number(l.driver_pay) + Number(l.fuel_cost),
      0,
    );
    const totalMiles = dayLoads.reduce((s, l) => s + Number(l.miles), 0);
    const delivered = dayLoads.filter(
    ...
    const byDriver: Record<
      string,
      { loads: number; miles: number; revenue: number; waitTotal: number }
    > = {};
    dayLoads.forEach((l) => {
      const d = driverName(l.driver_id);
      if (!byDriver[d])
        byDriver[d] = { loads: 0, miles: 0, revenue: 0, waitTotal: 0 };
      byDriver[d].loads += 1;
    ...
    const driverRows = Object.entries(byDriver)
      .map(([name, v]) => ({
        name,
        ...v,
        avgWait: v.loads ? Math.round(v.waitTotal / v.loads) : 0,
      }))
    ...
    return {
      ...
      shifts: [
        {
          label: "Day",
          loads: dayShift.length,
          miles: dayShift.reduce((s, l) => s + Number(l.miles), 0),
          revenue: dayShift.reduce((s, l) => s + Number(l.revenue), 0),
        },
        {
          label: "Night",
          loads: nightShift.length,
          miles: nightShift.reduce((s, l) => s + Number(l.miles), 0),
          revenue: nightShift.reduce((s, l) => s + Number(l.revenue), 0),
        },
      ],
    };
        ...
        {[
          {
            ...
            sub: `${dailyReport.delivered} delivered`,
          },
          {
            label: "Total Miles",
            value: dailyReport.totalMiles.toFixed(0),
            sub: "load miles",
          },
          {
            label: "Revenue",
            value: fmtMoney(dailyReport.totalRevenue),
            sub: "gross",
          },
          {
            label: "Costs",
            value: fmtMoney(dailyReport.totalCosts),
            sub: "driver + fuel",
          },
          {
            label: "Profit",
            value: fmtMoney(dailyReport.profit),
            sub: dailyReport.profit >= 0 ? "?" : "?",
          },
          {
            label: "Margin",
            value: `${dailyReport.margin.toFixed(1)}%`,
            sub:
              dailyReport.margin >= 30
                ? "Healthy"
                : dailyReport.margin >= 15
                  ? "OK"
                  : "Low",
          },
        ].map((s) => (
                ...
                {dailyReport.driverRows.map((d) => (
                  <TableRow key={d.name}>
                    ...
                    <TableCell className="text-right font-mono text-xs">
                      {d.miles > 0 ? fmtMoney(d.revenue / d.miles) : "--"}
                    </TableCell>
                    <TableCell>
                      <Badge className={`${waitBadgeClass(d.avgWait)} text-xs`}>
                        {fmtWait(d.avgWait)}
                      </Badge>
                    </TableCell>
                  </TableRow>
            ...
            <CardContent className="pt-5 pb-4 px-5">
              ...
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  ...
                  <p className="text-lg font-bold">{s.loads}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Miles</p>
                  <p className="text-lg font-bold">{s.miles.toFixed(0)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Revenue</p>
                  <p className="text-lg font-bold">{fmtMoney(s.revenue)}</p>
                </div>
              </div>
            </CardContent>
          </Card>
