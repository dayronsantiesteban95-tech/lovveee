...
export default function LoadBoard({
  ...
  const [activeTool, setActiveTool] = useState<
    "quick" | "import" | "history" | "log" | "blast" | null
  >("quick");
  const [autoDispatchLoadId, setAutoDispatchLoadId] = useState<string | null>(
    null,
  );
  const [clonePrefill, setClonePrefill] = useState<ReturnType<
    typeof cloneLoadData
  > | null>(null);
    ...
    <>
      <div className="flex items-center gap-3 mb-3 flex-wrap">
        <div className="flex items-center gap-2">
          <Label className="text-[10px] text-muted-foreground font-semibold uppercase tracking-wider whitespace-nowrap">
            Board Date
          </Label>
          <Input
            type="date"
      ...
      <div
        className={`grid gap-4 ${toolsOpen ? "grid-cols-1 lg:grid-cols-3" : "grid-cols-1"}`}
      >
        {/* Load Table */}
                        ...
                        {rawLoads.length > 0 ? (
                          <div className="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
                            ...
                            <p className="text-xs opacity-60">
                              No loads match your active filters.
                            </p>
                            <Button
                              variant="outline"
                              size="sm"
                          ...
                          <div className="flex flex-col items-center justify-center py-16 gap-2 text-muted-foreground">
                            ...
                            <p className="text-sm font-semibold text-foreground">
                              No loads for {selectedDate}
                            </p>
                            <p className="text-xs opacity-60">
                              Open the Tools panel &rarr; New to add the first
                              load for this date.
                            </p>
                          </div>
                        )}
                    ...
                    return (
                      ...
                      >
                        ...
                        <TableCell className="text-right font-mono text-sm">
                          {Number(load.miles).toFixed(0)}
                        </TableCell>
                        <TableCell className="text-right font-mono text-sm">
                          {Number(load.revenue) > 0
                            ? fmtMoney(Number(load.revenue))
                            : "--"}
                        </TableCell>
                        <TableCell>
                          {load.wait_time_minutes > 0 ? (
                            <Badge
                              className={`${waitBadgeClass(load.wait_time_minutes)} text-[10px]`}
                            >
                        ...
                        <TableCell>
                          ...
                          {load.status !== "in_progress" &&
                            load.eta_status &&
                            load.eta_status !== "unknown" && (
                              <span
                                className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-semibold rounded-full ${
                                  load.eta_status === "on_time"
                                    ? "status-on-time"
                                    : load.eta_status === "at_risk"
                                      ? "status-at-risk"
                                      : load.eta_status === "late"
                                        ? "status-late"
                                        : "status-idle"
                                }`}
                              >
                        ...
                        <TableCell onClick={(e) => e.stopPropagation()}>
                          <div className="flex flex-col gap-1.5 items-start">
                            {/* Status pill */}
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-semibold border ${si.pill ?? "bg-muted text-muted-foreground border-border"}`}
                            >
                              <span
                                className={`h-1.5 w-1.5 rounded-full ${si.color}`}
                              />
                              {si.label}
                            </span>
                            {/* Next-action button */}
                            {load.status === "pending" && (
                              <Button
                                ...
                                onClick={() =>
                                  onStatusChange(load.id, "assigned")
                                }
                              >
                            ...
                            {load.status === "assigned" && (
                              <Button
                                ...
                                onClick={() =>
                                  onStatusChange(load.id, "in_progress")
                                }
                              >
                            ...
                            {load.status === "in_progress" && (
                              <Button
                                ...
                                onClick={() =>
                                  onStatusChange(load.id, "delivered")
                                }
                              >
                            ...
                            {load.status === "arrived_pickup" && (
                              <Badge className="bg-blue-500/15 text-blue-500 border-0 text-[9px] h-5 px-1.5">
                                <MapPin className="h-2.5 w-2.5 mr-0.5" /> At
                                Pickup
                              </Badge>
                            )}
                            ...
                            {(load.status === "failed" ||
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  onStatusChange(load.id, "pending")
                                }
                              >
                          ...
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                toast({
                                  title: "Load cloned",
                                  description: "Edit and save the cloned load",
                                });
                              }}
                            >
                              ...
                              <DropdownMenuContent align="end" className="w-44">
                                          ...
                                          generateInvoice(
                                            {
                                              ...
                                              pickup_company:
                                                load.pickup_company ?? null,
                                              delivery_company:
                                                load.delivery_company ?? null,
                                              revenue: Number(load.revenue),
                                              packages: load.packages,
                                              weight_kg:
                                                (load as any).weight_kg ?? null,
                                              weight_lbs:
                                                load.weight_lbs ?? null,
                                              package_type:
                                                (load as any).package_type ??
                                                null,
                                              service_type: load.service_type,
                                              actual_pickup:
                                                load.actual_pickup ?? null,
                                              actual_delivery:
                                                load.actual_delivery ?? null,
                                ...
                                {STATUSES.map((s) => (
                                  <DropdownMenuItem
                                    ...
                                    onClick={() =>
                                      onStatusChange(load.id, s.value)
                                    }
                                  >
                                    <span
          ...
          <div className="space-y-4">
            ...
            {autoDispatchLoadId && (
              <AutoDispatchPanel
                ...
                loadPickupAddress={
                  loads.find((l) => l.id === autoDispatchLoadId)?.pickup_address
                }
                loadHub={loads.find((l) => l.id === autoDispatchLoadId)?.hub}
                onAssigned={() => {
                  onRefetchLoads();
                  setAutoDispatchLoadId(null);
                }}
            ...
            {activeTool === "log" && <ActivityLog compact />}
          </div>
        )}
      </div>
    </>
