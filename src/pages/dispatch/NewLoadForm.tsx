...
function calculateRate(
  ...
  const vt = (
    vehicleType === "box_truck" ? "box_truck" : "cargo_van"
  ) as AnikaVehicle;
...
export default function NewLoadForm({
  ...
  const [suggestedDriverId, setSuggestedDriverId] = useState<string | null>(
    null,
  );
  ...
  const [anikaModifiers, setAnikaModifiers] = useState<AnikaModifiers>(
  ...
  const suggestDriver = useCallback(
    (pickupAddress: string, driverList: Driver[]) => {
      ...
      const matchedHub = Object.entries(cityHubMap).find(([city]) =>
        addr.includes(city),
      )?.[1];
      if (!matchedHub) return;
      const match = driverList.find(
        (d) => d.hub === matchedHub && d.status === "active",
      );
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data }) => {
        if (data) setCompanyContacts(data as CompanyContact[]);
      });
  }, [addForm.client_id]);
  ...
  useEffect(() => {
    ...
    const card = rateCards.find(
      (r) =>
        r.hub === dbHub &&
        r.service_type === rateKey &&
        r.vehicle_type === addForm.vehicle_type,
    );
    if (!card) {
      setComputedRevenue(null);
      return;
  ...
  useEffect(() => {
    ...
    if (dim_l && dim_w && dim_h) {
      setAddForm((f) => ({
        ...f,
        dimensions_text: `${dim_l} x ${dim_w} x ${dim_h} CM`,
      }));
    }
  }, [addForm.dim_l, addForm.dim_w, addForm.dim_h]);

  const setAf = (patch: Partial<AddLoadForm>) =>
  ...
  const saveNewClient = async () => {
    ...
    if (error) {
      ...
      toast({
        title: "? Client saved",
        description: `${data.name} added to company database`,
      });
    }
  };
  ...
  const handleAddLoad = async () => {
    ...
    if (!addForm.driver_id) {
      toast({ title: "Driver assignment required", variant: "destructive" });
      return;
    }
    if (!addForm.pickup_address.trim()) {
      toast({ title: "Pickup address required", variant: "destructive" });
      return;
    }
    ...
    if (
      addForm.cutoff_time &&
      addForm.sla_deadline &&
      addForm.cutoff_time > addForm.sla_deadline
    ) {
      toast({
        title: "Cutoff time cannot be after SLA deadline",
        variant: "destructive",
      });
      return;
    }
    ...
    const [pickupCoords, deliveryCoords] = await Promise.all([
      ...
      addForm.delivery_address
        ? geocodeAddress(addForm.delivery_address)
        : null,
    ]);
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      ...
      console.warn(
        `[NewLoadForm] tracking_token collision on attempt ${attempt + 1}, retrying...`,
      );
      error = insertError;
    }
    ...
    if (error) {
      ...
      if (insertedLoad?.id) {
        try {
          ...
          console.warn(
            "[NewLoadForm] Failed to log initial status event:",
            evtErr,
          );
      ...
      if (insertedLoad?.id && addForm.driver_id) {
        try {
          ...
          console.warn(
            "[NewLoadForm] Push notification to driver failed:",
            pushErr,
          );
        }
      }
      ...
      toast({
        title: "? Load created!",
        description: `${addForm.reference_number} added to the board`,
      });
      onClose();
      onSuccess();
    }
  ...
  const companyAddresses = companies
    ...
    .map((c) => ({
      label: c.name,
      addr: [c.address, c.city, c.state].filter(Boolean).join(", "),
      id: c.id,
    }));
          ...
          {STEPS.map((label, i) => {
            ...
            return (
              <div key={step} className="flex items-center gap-0 flex-1">
                ...
                {i < STEPS.length - 1 && (
                  <div
                    className={`h-px flex-1 mx-3 ${done ? "bg-green-500" : "bg-border"}`}
                  />
                )}
              </div>
            );
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <Input
                      value={af.reference_number}
                      onChange={(e) =>
                        setAf({ reference_number: e.target.value })
                      }
                      placeholder="S01480980"
                      className="mt-1"
                    />
                  </div>
                  <div>
                    ...
                    <Input
                      value={af.consol_number}
                      onChange={(e) => setAf({ consol_number: e.target.value })}
                  ...
                  <div>
                    ...
                    <Input
                      value={af.po_number}
                      onChange={(e) => setAf({ po_number: e.target.value })}
                  ...
                  <div>
                    ...
                    <Input
                      type="datetime-local"
                      value={af.sla_deadline}
                      onChange={(e) => setAf({ sla_deadline: e.target.value })}
                      className="mt-1"
                    />
                  </div>
                  <div className="col-span-2">
                    ...
                    <Input
                      type="datetime-local"
                      value={af.cutoff_time}
                      onChange={(e) => setAf({ cutoff_time: e.target.value })}
                      className="mt-1"
                    />
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Airline cutoff -- maximum delivery time before flight is
                      missed
                    </p>
                  </div>
                </div>
                  ...
                  <div className="relative mt-1">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setAf({ client_name: e.target.value, client_id: "" });
                      }}
                    ...
                    <div className="absolute z-50 w-full mt-1 bg-popover border rounded-lg shadow-lg overflow-hidden">
                      <div className="max-h-52 overflow-y-auto">
                        {filteredCompanies.map((c) => (
                          <button
                            key={c.id}
                            className="w-full text-left px-3 py-2.5 hover:bg-muted/50 flex items-start gap-3 border-b border-border/50 last:border-0"
                            onClick={() => {
                      ...
                      {/* + New Client */}
                      <button
                        className="w-full text-left px-3 py-2.5 text-sm text-primary font-medium hover:bg-primary/5 flex items-center gap-2 border-t"
                        onClick={() => {
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mt-1">
                  <div>
                    ...
                    <Select
                      value={af.vehicle_type}
                      onValueChange={(v) => setAf({ vehicle_type: v })}
                    >
                      <SelectTrigger className="mt-1">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {VEHICLE_TYPES_DISPATCH.map((v) => (
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      step="0.1"
                      value={af.distance_miles}
                      onChange={(e) =>
                        setAf({ distance_miles: e.target.value })
                      }
                      placeholder="31"
                      className="mt-1"
                    />
                  ...
                  <div className="p-4 rounded-xl border border-emerald-500/30 bg-emerald-500/5 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-semibold uppercase tracking-wider text-emerald-700 dark:text-emerald-400 flex items-center gap-1.5">
                        <DollarSign className="h-3.5 w-3.5" /> Live Rate
                        Estimate
                      </p>
                    ...
                    {miles === 0 && (
                      <p className="text-[10px] text-muted-foreground text-center">
                        Enter distance above to see mileage charge
                      </p>
                    )}
                    <Button
                      size="sm"
                    ...
                    >
                      <DollarSign className="h-3.5 w-3.5" /> Use this rate: $
                      {bd.subtotal.toFixed(2)}
                    </Button>
                  </div>
                );
                ...
                <div className="mt-1">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={
                        computedRevenue ? computedRevenue.toFixed(2) : "0.00"
                      }
                      className="pl-7"
                    />
                  </div>
                    ...
                    <div className="mt-2 flex items-center gap-2 p-2.5 rounded-lg bg-emerald-500/10 border border-emerald-500/20">
                      ...
                      <div className="flex-1">
                        <p className="text-xs text-emerald-700 dark:text-emerald-400 font-medium">
                          Rate card estimate:{" "}
                          <strong>${computedRevenue.toFixed(2)}</strong>
                        </p>
                        <p className="text-[10px] text-muted-foreground">
                          {af.service_type} ?{" "}
                          {
                            VEHICLE_TYPES_DISPATCH.find(
                              (v) => v.value === af.vehicle_type,
                            )?.label
                          }{" "}
                          ? {af.distance_miles || 0} mi
                        </p>
                      </div>
                      <Button
                        size="sm"
                        ...
                        onClick={() =>
                          setAf({ revenue: computedRevenue.toFixed(2) })
                        }
                      >
                        Use
                      </Button>
          ...
          {addStep === 2 && (
            <div className="space-y-5">
              ...
              <div className="p-4 rounded-xl border border-green-500/30 bg-green-500/5">
                <p className="form-section-label text-green-700 dark:text-green-400">
                  ?? Pickup Location
                </p>
                  ...
                  <div className="relative mt-1">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setAf({ pickup_company: e.target.value });
                      }}
                      ...
                      <div className="absolute z-50 w-full mt-1 bg-popover border rounded-lg shadow-lg overflow-hidden">
                        <div className="max-h-44 overflow-y-auto">
                          {recentAddresses.slice(0, 3).map((addr, i) => (
                            <button
                              key={`r${i}`}
                              className="w-full text-left px-3 py-2 hover:bg-muted/50 flex items-center gap-2 text-xs border-b border-border/40"
                              onClick={() => {
                                setPickupSearch(addr);
                                setAf({ pickup_address: addr });
                          ...
                          {pickupSuggestions.map((s) => (
                            <button
                              key={s.id}
                              className="w-full text-left px-3 py-2.5 hover:bg-muted/50 flex items-start gap-2 border-b border-border/40 last:border-0"
                              onClick={() => {
                                setPickupSearch(s.label);
                                setAf({
                                  pickup_company: s.label,
                                  pickup_address: s.addr,
                                  pickup_contact_name: companyContacts.find(
                                    (c) => c.company_id === s.id,
                                  )
                                    ? `${companyContacts.find((c) => c.company_id === s.id)!.first_name} ${companyContacts.find((c) => c.company_id === s.id)!.last_name}`
                                    : af.pickup_contact_name,
                                  pickup_contact_phone:
                                    companyContacts.find(
                                      (c) => c.company_id === s.id,
                                    )?.phone ?? af.pickup_contact_phone,
                                });
                                setShowPickupDropdown(false);
                              }}
                            >
                ...
                <div className="grid grid-cols-1 gap-3 mt-3">
                  <div>
                    ...
                    <Input
                      value={af.pickup_address}
                      onChange={(e) =>
                        setAf({ pickup_address: e.target.value })
                      }
                      placeholder="123 Airport Cargo Rd, Phoenix, AZ 85034"
                      className="mt-1"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        value={af.pickup_open_hours}
                        onChange={(e) =>
                          setAf({ pickup_open_hours: e.target.value })
                        }
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        ...
                        <Input
                          type="time"
                          value={af.pickup_time_from}
                          onChange={(e) =>
                            setAf({ pickup_time_from: e.target.value })
                          }
                          className="mt-1"
                        />
                      </div>
                      <div>
                        ...
                        <Input
                          type="time"
                          value={af.pickup_time_to}
                          onChange={(e) =>
                            setAf({ pickup_time_to: e.target.value })
                          }
                          className="mt-1"
                        />
                      </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Input
                        value={af.pickup_contact_name}
                        onChange={(e) =>
                          setAf({ pickup_contact_name: e.target.value })
                        }
                    ...
                    <div>
                      ...
                      <Input
                        value={af.pickup_contact_phone}
                        onChange={(e) =>
                          setAf({ pickup_contact_phone: e.target.value })
                        }
                        placeholder="602-555-0100"
                        className="mt-1"
                      />
              ...
              <div className="p-4 rounded-xl border border-orange-500/30 bg-orange-500/5">
                <p className="form-section-label text-orange-700 dark:text-orange-400">
                  ?? Delivery Location
                </p>
                  ...
                  <div className="relative mt-1">
                    <Input
                      ...
                      onChange={(e) => {
                        ...
                        setAf({ delivery_company: e.target.value });
                      }}
                        ...
                        <div className="max-h-44 overflow-y-auto">
                          {recentAddresses.slice(0, 3).map((addr, i) => (
                            <button
                              ...
                              className="w-full text-left px-3 py-2 hover:bg-muted/50 flex items-center gap-2 text-xs border-b border-border/40"
                              onClick={() => {
                                setDeliverySearch(addr);
                                setAf({ delivery_address: addr });
                            ...
                            <button
                              ...
                              onClick={() => {
                                ...
                                setAf({
                                  ...
                                  delivery_contact_name: companyContacts.find(
                                    (c) => c.company_id === s.id,
                                  )
                                    ? `${companyContacts.find((c) => c.company_id === s.id)!.first_name} ${companyContacts.find((c) => c.company_id === s.id)!.last_name}`
                                    : af.delivery_contact_name,
                                  delivery_contact_phone:
                                    companyContacts.find(
                                      (c) => c.company_id === s.id,
                                    )?.phone ?? af.delivery_contact_phone,
                                });
                                setShowDeliveryDropdown(false);
                              }}
                            >
                ...
                <div className="grid grid-cols-1 gap-3 mt-3">
                  <div>
                    <Label className="text-xs">Street Address *</Label>
                    <Input
                      value={af.delivery_address}
                      onChange={(e) =>
                        setAf({ delivery_address: e.target.value })
                      }
                      placeholder="456 Cargo Way, Scottsdale, AZ 85257"
                      className="mt-1"
                    />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <Label className="text-xs">Contact Name</Label>
                      <Input
                        value={af.delivery_contact_name}
                        onChange={(e) =>
                          setAf({ delivery_contact_name: e.target.value })
                        }
                    ...
                    <div>
                      <Label className="text-xs">Contact Phone</Label>
                      <Input
                        value={af.delivery_contact_phone}
                        onChange={(e) =>
                          setAf({ delivery_contact_phone: e.target.value })
                        }
                        placeholder="480-555-0200"
                        className="mt-1"
                      />
                    </div>
                  </div>
          ...
          {addStep === 3 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3 mt-1">
                  <div>
                    ...
                    <Input
                      ...
                      min="1"
                      value={af.packages}
                      onChange={(e) => setAf({ packages: e.target.value })}
                      className="mt-1"
                    />
                  </div>
                  <div>
                    ...
                    <Select
                      value={af.package_type}
                      onValueChange={(v) => setAf({ package_type: v })}
                    >
                      <SelectTrigger className="mt-1">
                        <SelectValue />
                      </SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      step="0.1"
                      value={af.weight_kg}
                      onChange={(e) => setAf({ weight_kg: e.target.value })}
                      placeholder="0.0"
                      className="mt-1"
                    />
                  </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <Input
                      type="number"
                      value={af.dim_l}
                      onChange={(e) => setAf({ dim_l: e.target.value })}
                      placeholder="L"
                    ...
                    <Input
                      type="number"
                      value={af.dim_w}
                      onChange={(e) => setAf({ dim_w: e.target.value })}
                      placeholder="W"
                      className="text-center"
                    />
                    <span className="text-muted-foreground text-sm font-medium shrink-0">
                    ...
                    <Input
                      type="number"
                      value={af.dim_h}
                      onChange={(e) => setAf({ dim_h: e.target.value })}
                      placeholder="H"
                      className="text-center"
                    />
                  </div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1.5">
                      ...
                      {af.weight_kg && (
                        <>
                          {" "}
                          ? Dim wt:{" "}
                          <strong>{(cubic / 5000).toFixed(1)} kg</strong>
                        </>
                      )}
                      {af.dimensions_text && (
                        <>
                ...
                <div className="mt-3">
                  ...
                  <Input
                    ...
                    placeholder="e.g. CIVIL AIRCRAFT PART -- LH ENGINE SEAL"
                    className="mt-1"
                  />
                </div>
              </div>
              ...
              {(af.vehicle_type === "cargo_van" ||
                af.vehicle_type === "box_truck") &&
                (() => {
                  const miles = parseFloat(af.distance_miles) || 0;
                  const wKg = parseFloat(af.weight_kg) || 0;
                  const wLbs = wKg * 2.205;
                  const bd = calculateRate(
                    af.vehicle_type,
                    miles,
                    wLbs,
                    anikaModifiers,
                  ...
                  return (
                    <div className="p-4 rounded-xl border border-blue-500/30 bg-blue-500/5 space-y-4">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {[
                            ...
                            {
                              key: "tenderingFee" as const,
                              label: "Tendering Fee (airport)",
                              price: 15,
                            },
                            {
                              key: "attemptCharge" as const,
                            ...
                            >
                              <div className="flex items-center gap-2">
                                <Checkbox
                                  id={`mod-${key}`}
                                  checked={anikaModifiers[key] as boolean}
                                  onCheckedChange={(v) =>
                                    setMod({ [key]: !!v })
                                  }
                                />
                        ...
                        <div className="flex items-center justify-between gap-2 p-2 rounded-lg bg-background/60">
                          ...
                          <div className="flex items-center gap-2">
                            <Button
                              ...
                              onClick={() =>
                                setMod({
                                  additionalStops: Math.max(
                                    0,
                                    anikaModifiers.additionalStops - 1,
                                  ),
                                })
                              }
                            >
                            ...
                            <span className="text-sm font-mono w-5 text-center">
                              {anikaModifiers.additionalStops}
                            </span>
                            <Button
                              type="button"
                              variant="outline"
                      ...
                      <div className="space-y-2">
                        ...
                        {[
                          {
                            key: "extraPieces" as const,
                            label: "Extra Pieces (+$15 each)",
                            unit: 15,
                          },
                          {
                            key: "holding" as const,
                            label: "Holding per day (+$50 each)",
                            unit: 50,
                          },
                          {
                            key: "waitTime" as const,
                            label:
                              "Wait Time per 15 min (+$30 each, first free)",
                            unit: 30,
                          },
                        ].map(({ key, label }) => (
                          <div
                          ...
                          >
                            ...
                            <div className="flex items-center gap-2">
                              <Button
                                ...
                                onClick={() =>
                                  setMod({
                                    [key]: Math.max(
                                      0,
                                      (anikaModifiers[key] as number) - 1,
                                    ),
                                  })
                                }
                              >
                              ...
                              <Button
                                ...
                                onClick={() =>
                                  setMod({
                                    [key]: (anikaModifiers[key] as number) + 1,
                                  })
                                }
                              >
                                +
                      ...
                      <div className="space-y-1.5">
                        ...
                        <div className="flex justify-between items-center p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/30 mt-2">
                          ...
                          <span className="text-xl font-bold text-emerald-700 dark:text-emerald-400 font-mono">
                            ${bd.finalQuote.toFixed(2)}
                          </span>
                        </div>
                        <Button
                          size="sm"
                          className="w-full h-8 text-xs gap-1.5 bg-emerald-600 hover:bg-emerald-700 text-white mt-1"
                          onClick={() =>
                            setAf({ revenue: bd.finalQuote.toFixed(2) })
                          }
                        >
                          <DollarSign className="h-3.5 w-3.5" /> Apply to
                          Revenue: ${bd.finalQuote.toFixed(2)}
                        </Button>
                      </div>
                    </div>
                  );
              ...
              <div>
                ...
                <div className="space-y-2 mt-1">
                  ...
                  {drivers.map((d) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`h-9 w-9 rounded-full flex items-center justify-center text-sm font-bold shrink-0 ${isSelected ? "bg-primary text-primary-foreground" : "bg-muted"}`}
                        >
                          {d.full_name
                            .split(" ")
                            .map((n) => n[0])
                ...
                {suggestedDriverId && !af.driver_id && (
                  <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
                    <Zap className="h-3 w-3" /> AI matched a driver based on
                    pickup location. Review and confirm above.
                  </p>
                )}
              </div>
              ...
              <div className="p-4 rounded-xl bg-muted/30 border space-y-2 text-sm">
                ...
                <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs">
                  ...
                  <span>{af.service_type}</span>
                  <span className="text-muted-foreground">Revenue</span>
                  <span className="font-semibold text-green-600">
                    $
                    {af.revenue
                      ? parseFloat(af.revenue).toFixed(2)
                      : computedRevenue
                        ? computedRevenue.toFixed(2)
                        : "0.00"}
                  </span>
                  <span className="text-muted-foreground">Pickup</span>
                  <span className="truncate">
                    {af.pickup_company || af.pickup_address || "--"}
                  </span>
                  <span className="text-muted-foreground">Delivery</span>
                  <span className="truncate">
                    {af.delivery_company || af.delivery_address || "--"}
                  </span>
                  <span className="text-muted-foreground">Driver</span>
                  <span>
                    {drivers.find((d) => d.id === af.driver_id)?.full_name || (
                      <span className="text-red-500">Not assigned</span>
                    )}
                  </span>
                  <span className="text-muted-foreground">BOL</span>
                  <span>{bolFile ? bolFile.name : "--"}</span>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t bg-muted/10 shrink-0">
          ...
          <div className="flex items-center gap-2">
            {addStep < 3 ? (
              ...
              <Button
                className="btn-gradient gap-1.5"
                disabled={!canSubmit || bolUploading}
