...
export default function WaitTimeTab({
  analytics,
  driverName,
}: WaitTimeTabProps) {
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          {
            label: "Avg Wait (Period)",
            value: fmtWait(analytics.avgAll),
            icon: Clock,
            color: analytics.avgAll > 30 ? "text-red-500" : "text-green-500",
          },
          {
            label: "Detention Eligible",
            value: analytics.detentionEligible.length,
            icon: AlertTriangle,
            color: "text-orange-500",
          },
          {
            label: "Detention Billed",
            value: fmtMoney(analytics.detentionBilled),
            icon: DollarSign,
          ...
          },
          {
            ...
            value:
              analytics.detentionEligible.length > 0
                ? `${Math.round((analytics.detentionEligible.filter((l) => l.detention_billed > 0).length / analytics.detentionEligible.length) * 100)}%`
                : "N/A",
            icon: BarChart3,
            color: "text-blue-500",
          },
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="glass-card rounded-2xl">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Wait Time by Client</CardTitle>
          </CardHeader>
          <CardContent>
                  ...
                  <Bar dataKey="avg" radius={[0, 4, 4, 0]}>
                    {analytics.clientWait.map((_, i) => (
                      <Cell
                        key={i}
                        fill={
                          analytics.clientWait[i].avg >= 30
                            ? "hsl(0,70%,55%)"
                            : analytics.clientWait[i].avg >= 15
                              ? "hsl(40,90%,50%)"
                              : "hsl(140,60%,45%)"
                        }
                      />
        ...
        <Card className="glass-card rounded-2xl">
          <CardHeader className="pb-2">
            <CardTitle className="text-base">Wait Time by Driver</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      <Card className="glass-card rounded-2xl">
        <CardHeader className="pb-2">
          <CardTitle className="text-base flex items-center gap-2">
            ...
            Detention-Eligible Loads (?{DETENTION_THRESHOLD}min)
          </CardTitle>
        </CardHeader>
        <CardContent>
          {analytics.detentionEligible.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-6">
              No detention-eligible loads in this period ??
            </p>
          ) : (
            <Table>
              <TableHeader>
              ...
              <TableBody>
                {analytics.detentionEligible.map((l) => (
                  <TableRow key={l.id}>
                    ...
                    <TableCell>
                      <Badge
                        className={`${waitBadgeClass(l.wait_time_minutes)} text-xs`}
                      >
                        {fmtWait(l.wait_time_minutes)}
                      </Badge>
                    </TableCell>
                    <TableCell className="text-right font-mono">
                      {l.detention_billed > 0 ? (
                        fmtMoney(l.detention_billed)
                      ) : (
                        <span className="text-red-500 text-xs">Not billed</span>
                      )}
                    </TableCell>
                  </TableRow>
                ))}
