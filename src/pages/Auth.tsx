...
function Auth() {
  ...
  const handleSetNewPassword = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast({
        ...
        description: "Your new password has been set. Welcome!",
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-primary/5 via-background to-accent/5 p-4 relative overflow-hidden">
      ...
      <div
        className="absolute bottom-1/4 -right-32 w-80 h-80 bg-primary/10 rounded-full blur-3xl animate-float"
          ...
          <CardContent>
            {needsPasswordChange ? (
              ...
              <form onSubmit={handleSetNewPassword} className="space-y-4">
                ...
                <Button
                  type="submit"
                  className="w-full h-11 btn-gradient text-white font-medium pulse-glow"
                  disabled={loading}
                >
                  {loading ? (
                    "Updating..."
                    ...
                    <>
                      <ShieldCheck className="h-4 w-4 mr-2" /> Set Password &
                      Continue
                    </>
                  )}
                </Button>
