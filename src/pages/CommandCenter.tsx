...
function MetricCard({
  ...
  return (
    <div
      className={`metric-card p-4 ${compact ? "p-3" : ""} ${glowClasses[variant]} animate-in`}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-1.5">
          ...
          <p
            className={`text-2xl font-bold text-data tracking-tight ${compact ? "text-xl" : ""} ${variantClasses[variant]}`}
          >
            {value}
          </p>
          {trend && (
        ...
        <div
          className={`relative p-2 rounded-lg ${variant === "live" ? "bg-emerald-500/10" : "bg-muted/50"}`}
        >
...
function LiveDot({
  ...
  const sizeClass = { xs: "h-1.5 w-1.5", sm: "h-2 w-2", md: "h-2.5 w-2.5" }[
    size
  ];
  ...
  return (
    <span className="relative flex">
      {isLive && (
        <span
          className={`animate-marker-ping absolute inline-flex ${sizeClass} rounded-full ${colors[status] ?? "bg-muted-foreground"} opacity-75`}
        />
      )}
      <span
        className={`relative inline-flex rounded-full ${sizeClass} ${colors[status] ?? "bg-muted-foreground"}`}
      />
    </span>
  );
}
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2 py-0.5 text-[10px] font-semibold rounded-full tracking-wide uppercase ${config.className}`}
    >
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5">
          ...
          {severity === "critical" && (
            <span className="text-[9px] px-1 py-0.5 rounded bg-red-500/15 text-red-400 font-semibold">
              CRITICAL
            </span>
          )}
        </div>
  ...
  return (
    <div className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-muted/30 transition-colors group cursor-pointer">
      <div className="relative">
        ...
        <span
          className={`absolute -bottom-0.5 -right-0.5 h-2.5 w-2.5 rounded-full border-2 border-card ${statusColor}`}
        />
      </div>
      <div className="flex-1 min-w-0">
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] text-muted-foreground truncate">
          {driverName} * {Number(load.miles).toFixed(0)} mi * $
          {Number(load.revenue).toFixed(0)}
        </p>
      </div>
      <ChevronRight className="h-3.5 w-3.5 text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity" />
    </div>
    ...
    <div className="absolute inset-0 bg-[hsl(224,40%,4%)] flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center space-y-6">
        ...
        <div className="relative mx-auto w-32 h-32">
          ...
          <div
            className="absolute inset-4 rounded-full border border-primary/15 animate-marker-ping"
            style={{ animationDelay: "0.5s" }}
          />
          <div
            className="absolute inset-8 rounded-full border border-primary/10 animate-marker-ping"
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-muted-foreground max-w-md">
            Live driver positions, load routes, and status overlays will render
            on the map.
            <br />
            Add{" "}
            <code className="text-data text-primary text-xs">
              GOOGLE_MAPS_API_KEY
            </code>{" "}
            to enable.
          </p>
...
function CommandCenter() {
  ...
  const hubs = useMemo(
    () => [...new Set(loads.map((l) => l.hub).filter(Boolean))],
    [loads],
  );
  ...
  const metrics = useMemo(() => {
    ...
    const atRisk = loads.filter(
      (l) => l.wait_time_minutes > 30 && l.status !== "delivered",
    ).length;
  ...
  const combinedAlerts = useMemo(() => {
    ...
    const unassignedLoads = loads.filter(
      (l) =>
        !l.driver_id && l.status !== "delivered" && l.status !== "cancelled",
    );
    unassignedLoads.forEach((l) => {
      ...
      dbAlerts.push({
        ...
        ageMinutes: Math.round(
          (Date.now() - new Date(l.created_at).getTime()) / 60_000,
        ),
      });
    });
      ...
      <div className="cc-metrics-bar">
        <div className="flex-1 grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-2">
          ...
          <div className="metric-card p-3 flex flex-col justify-between col-span-2 md:col-span-1">
            <div>
              <p className="text-[11px] font-bold text-foreground tracking-tight leading-tight">
                Live Ops
              </p>
            ...
            <div className="flex items-center gap-1.5">
              {[
                ...
                >
                  ...
                  {tab.count > 0 && (
                    <span
                      className={`text-data text-[9px] ml-0.5 ${sidebarView === tab.key ? "text-primary-foreground/70" : "text-muted-foreground"}`}
                    >
                      {tab.count}
                    </span>
                  )}
              ...
              <div className="flex gap-1.5">
                <Select value={filterStatus} onValueChange={setFilterStatus}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="blasted" className="text-xs">
                      Blasted
                    </SelectItem>
                  </SelectContent>
                </Select>
            ...
            {!loading && sidebarView === "loads" && (
              <>
                ...
                {filteredLoads.map((load, i) => (
                  <div
                    key={load.id}
                    className={`stagger-${Math.min(i + 1, 6)} animate-in`}
                  >
                    <LoadRow
                      load={load}
                  ...
                  >
                    <AlertItem
                      ...
                      onBlast={(loadId) => {
                        ...
                        toast({
                          title: "Opening Blast Panel...",
                          description: "Select the load to blast",
                        });
                      }}
                    />
                  </div>
          ...
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-[11px] font-semibold text-foreground">
                {combinedAlerts.length} Alert
                {combinedAlerts.length !== 1 ? "s" : ""}
              </span>
