...
function normalizeChecklistItem(val: unknown): {
  result: "pass" | "issue" | null;
  note: string | null;
} {
  if (val === null || val === undefined) return { result: null, note: null };
  if (typeof val === "boolean")
    return { result: val ? "pass" : "issue", note: null };
  if (typeof val === "object" && val !== null && "result" in val)
    return val as { result: "pass" | "issue" | null; note: string | null };
...
function FleetTracker() {
  ...
  const loading =
    vehiclesLoading || maintLoading || driversLoading || fleetStatusLoading;
  ...
  const openHistory = useCallback(
    async (vehicle: Vehicle) => {
      ...
      try {
        ...
        if (odo.error) {
          toast({
            title: "Error loading odometer history",
            description: odo.error.message,
            variant: "destructive",
          });
        } else if (odo.data) {
        ...
        if (insp.error) {
          toast({
            ...
            description: insp.error.message,
            variant: "destructive",
          });
        } else if (insp.data) {
  ...
  const flagInspection = useCallback(
  ...
  const stats = useMemo(() => {
    ...
    const expiringInsurance = vehicles.filter(
      (v) =>
        v.insurance_expiry &&
    ...
    if (vinValue) {
      ...
      if (!vinRegex.test(vinValue)) {
        toast({
          title: "Invalid VIN",
          description:
            "VIN must be exactly 17 alphanumeric characters (no I, O, or Q).",
          variant: "destructive",
        });
        return;
      }
  ...
  const handleMaintSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!vehicleId) {
      toast({
        ...
        description: "Please select a vehicle for this maintenance record.",
        variant: "destructive",
      });
      return;
    }
    ...
    const payload = {
      ...
      mileage_at_service:
        rawMileageAtService !== "" ? Number(rawMileageAtService) : null,
      service_date:
        (fd.get("service_date") as string) ||
        new Date().toISOString().split("T")[0],
      next_service_date: (fd.get("next_service_date") as string) || null,
      next_service_mileage:
        rawNextServiceMileage !== "" ? Number(rawNextServiceMileage) : null,
    ...
    if (error)
      ...
      toast({ title: editMaint ? "Record updated" : "Maintenance logged" });
  ...
  const handleDrvSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error)
      ...
      toast({ title: editDrv ? "Driver updated" : "Driver added" });
      setDrvDialog(false);
      setEditDrv(null);
      refetchAll();
    }
  ...
  const vehStatusBadge = (s: string) =>
    VEHICLE_STATUSES.find((v) => v.value === s) ?? VEHICLE_STATUSES[0];
  const drvStatusBadge = (s: string) =>
    DRIVER_STATUSES.find((d) => d.value === s) ?? DRIVER_STATUSES[0];
  const vehTypeLbl = (t: string) =>
    VEHICLE_TYPES.find((v) => v.value === t)?.label ?? t;
  const maintTypeLbl = (t: string) =>
    MAINT_TYPES.find((m) => m.value === t)?.label ?? t;
    ...
    <div className="space-y-6 animate-in">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Vehicle management, maintenance scheduling & driver roster
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-4">
        {[
          ...
          {
            label: "Service Due",
            value: stats.serviceDue.length,
            icon: AlertTriangle,
            color:
              stats.serviceDue.length > 0 ? "text-red-500" : "text-green-500",
          },
          {
            label: "Insurance Expiring",
            value: stats.expiringInsurance.length,
            icon: Shield,
            color:
              stats.expiringInsurance.length > 0
                ? "text-orange-500"
                : "text-green-500",
          },
          {
          ...
          {
            label: "Maint. Costs",
            value: fmtMoney(stats.totalMaintCost),
            icon: Wrench,
            color: "text-red-500",
          },
        ].map((s) => (
          ...
          <CardContent className="pt-0">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              {[
                ...
                },
                {
                  ...
                  value: inspectionStats.inspected,
                  color: "text-green-500",
                },
                {
                ...
                {
                  label: "Car Wash Overdue",
                  value: inspectionStats.washOverdue,
                  color:
                    inspectionStats.washOverdue > 0
                      ? "text-orange-500"
                      : "text-green-500",
                },
              ].map((s) => (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
              {fleetStatus.map((fs) => (
                <div
                  key={fs.vehicle_id}
                  className="flex items-center justify-between rounded-lg bg-muted/30 px-3 py-2 gap-2"
                >
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <InspectionBadge
                      done={fs.inspection_done}
                      status={fs.inspection_status}
                    />
                    {fs.car_wash_overdue && (
      ...
      <Tabs defaultValue="vehicles" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="maintenance" className="gap-1.5">
            <Wrench className="h-3.5 w-3.5" /> Maintenance ({maintenance.length}
            )
          </TabsTrigger>
          <TabsTrigger value="drivers" className="gap-1.5">
            <Users className="h-3.5 w-3.5" /> Drivers ({drivers.length})
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="vehicles" className="space-y-4">
          <div className="flex gap-2 justify-end">
            <Button
              ...
              className="gap-2"
              onClick={() => {
                setSelectedVehicleForInspection(undefined);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {vehicles.map((v) => {
              ...
              const serviceDue =
                (v.next_service_date &&
                  new Date(v.next_service_date) <= today) ||
                (v.next_service_mileage &&
                  v.current_mileage >= v.next_service_mileage);
              const insuranceWarn =
                v.insurance_expiry &&
                differenceInDays(new Date(v.insurance_expiry), today) <= 30;
              const fs = getVehicleFleetStatus(v.id);
              return (
                <Card
                  key={v.id}
                  className={`glass-card rounded-2xl relative overflow-hidden ${serviceDue ? "ring-2 ring-red-500/30" : ""}`}
                >
                  {serviceDue && (
                  ...
                  <CardContent className="pt-5 pb-4 px-5">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <Badge className={`${stBadge.badge} text-[10px]`}>
                        {stBadge.label}
                      </Badge>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-sm mb-3">
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        ...
                        {(v.current_mileage || 0).toLocaleString()} mi
                      </div>
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <Fuel className="h-3 w-3" />
                        {v.avg_mpg ? `${v.avg_mpg} mpg` : "--"}
                      </div>
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                        <Truck className="h-3 w-3" />
                        {vehTypeLbl(v.vehicle_type)}
                      </div>
                      <div className="flex items-center gap-1.5 text-muted-foreground">
                    ...
                    {fs && (
                      <div className="space-y-1.5 mb-3">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          />
                          <CarWashBadge
                            daysSince={fs.days_since_wash}
                            overdue={fs.car_wash_overdue}
                          />
                        </div>
                        {fs.last_odometer != null && (
                          <p className="text-[10px] text-muted-foreground flex items-center gap-1">
                            <Gauge className="h-2.5 w-2.5" />
                            Last odometer: {fs.last_odometer.toLocaleString()}{" "}
                            mi
                          </p>
                        )}
                      </div>
                    ...
                    <div className="flex gap-1 mt-2 flex-wrap">
                      ...
                      <Button
                        ...
                        className="h-7 text-xs gap-1"
                        onClick={() => {
                          setSelectedVehicleForInspection(v.id);
                          setInspectionDialog(true);
                        }}
                      >
                      ...
                      <TableCell colSpan={8}>
                        <div className="flex flex-col items-center justify-center py-14 gap-3 text-muted-foreground">
                          ...
                          <p className="text-xs opacity-60">
                            Log your first service record to start tracking
                            vehicle health.
                          </p>
                        </div>
                      </TableCell>
                  ...
                  {maintenance.map((m) => (
                    <TableRow key={m.id}>
                      ...
                      <TableCell className="font-medium text-sm">
                        {vehName(m.vehicle_id)}
                      </TableCell>
                      <TableCell>
                        <Badge variant="secondary" className="text-[10px]">
                          {maintTypeLbl(m.maintenance_type)}
                        </Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-right font-mono text-sm">
                        {fmtMoney(Number(m.cost))}
                      </TableCell>
                      <TableCell className="font-mono text-xs">
                        {m.mileage_at_service?.toLocaleString() ?? "--"}
                      </TableCell>
                      <TableCell className="text-sm">
                        {m.vendor || "--"}
                      </TableCell>
                      <TableCell>
                        <div className="flex gap-1">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              setDeleteTarget({ type: "maintenance", id: m.id })
                            }
                          >
                            <Trash2 className="h-3 w-3" />
                          </Button>
                ...
                <TableBody>
                    ...
                    <TableRow>
                      <TableCell colSpan={8}>
                        <div className="flex flex-col items-center justify-center py-14 gap-3 text-muted-foreground">
                          ...
                          <p className="text-xs opacity-60">
                            Add your first driver to start assigning loads and
                            tracking shifts.
                          </p>
                        </div>
                      </TableCell>
                  ...
                  {drivers.map((d) => {
                    ...
                    const licenseWarn =
                      d.license_expiry &&
                      differenceInDays(
                        new Date(d.license_expiry),
                        new Date(),
                      ) <= 30;
                    return (
                      <TableRow
                        key={d.id}
                        className={licenseWarn ? "bg-orange-500/5" : ""}
                      >
                        <TableCell className="font-medium">
                          {d.full_name}
                        </TableCell>
                        <TableCell className="text-sm">
                          {HUBS.find((h) => h.value === d.hub)?.label ?? d.hub}
                        </TableCell>
                        <TableCell className="text-sm">
                          {d.phone || "--"}
                        </TableCell>
                        <TableCell>
                          <Badge className={`${dBadge.badge} text-[10px]`}>
                            {dBadge.label}
                          </Badge>
                        </TableCell>
                        <TableCell className="text-sm">
                          {d.license_expiry ? (
                            <span
                              className={
                                licenseWarn ? "text-orange-500 font-medium" : ""
                              }
                            >
                              {d.license_expiry}
                              {licenseWarn && " (expiring soon)"}
                            </span>
                          ) : (
                            "--"
                          )}
                        ...
                        <TableCell>
                          <div className="flex gap-1 items-center">
                            ...
                            <Button
                              ...
                              className="h-7 w-7 text-destructive"
                              onClick={() =>
                                setDeleteTarget({ type: "driver", id: d.id })
                              }
                            >
                              <Trash2 className="h-3 w-3" />
                            </Button>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              <div>
                ...
                <Input
                  name="vehicle_name"
                  required
                  defaultValue={editVeh?.vehicle_name ?? ""}
                  placeholder="CV-001"
                />
              </div>
              <div>
                ...
                <Select
                  name="vehicle_type"
                  defaultValue={editVeh?.vehicle_type ?? "cargo_van"}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                <Label>Hub</Label>
                <Select name="hub" defaultValue={editVeh?.hub ?? "phoenix"}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Input
                  name="year"
                  type="number"
                  defaultValue={editVeh?.year ?? ""}
                  placeholder="2024"
                />
              </div>
              <div>
                <Label>VIN</Label>
                <Input name="vin" defaultValue={editVeh?.vin ?? ""} />
              </div>
              <div>
                <Label>License Plate</Label>
                <Input
                  name="license_plate"
                  defaultValue={editVeh?.license_plate ?? ""}
                />
              </div>
              <div>
                <Label>Status</Label>
                <Select
                  name="status"
                  defaultValue={editVeh?.status ?? "active"}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  type="number"
                  defaultValue={editVeh?.current_mileage ?? 0}
                />
              </div>
              <div>
                <Label>Next Service (mi)</Label>
                <Input
                  name="next_service_mileage"
                  type="number"
                  defaultValue={editVeh?.next_service_mileage ?? ""}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  type="date"
                  defaultValue={editVeh?.next_service_date ?? ""}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  type="date"
                  defaultValue={editVeh?.insurance_expiry ?? ""}
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  type="date"
                  defaultValue={editVeh?.registration_expiry ?? ""}
                />
              </div>
              <div>
                ...
                <Input
                  name="fuel_type"
                  defaultValue={editVeh?.fuel_type ?? "gasoline"}
                />
              </div>
              <div>
                <Label>Avg MPG</Label>
              ...
              <div>
                ...
                <Input
                  ...
                  type="number"
                  step="0.01"
                  defaultValue={editVeh?.daily_rate ?? 0}
                />
              </div>
            </div>
            <div>
      ...
      <Dialog
        ...
        onOpenChange={(o) => {
          setMaintDialog(o);
          if (!o) setEditMaint(null);
        }}
      >
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {editMaint ? "Edit Maintenance" : "Log Maintenance"}
            </DialogTitle>
            <DialogDescription>
              Record a service or repair for a vehicle.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleMaintSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select
                  name="vehicle_id"
                  defaultValue={editMaint?.vehicle_id ?? ""}
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  defaultValue={editMaint?.maintenance_type ?? "oil_change"}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  type="number"
                  defaultValue={editMaint?.mileage_at_service ?? ""}
                />
              </div>
              <div>
                <Label>Vendor</Label>
                <Input name="vendor" defaultValue={editMaint?.vendor ?? ""} />
              </div>
              <div>
                <Label>Next Service Date</Label>
                <Input
                  name="next_service_date"
                  type="date"
                  defaultValue={editMaint?.next_service_date ?? ""}
                />
              </div>
              <div>
                <Label>Next Service (mi)</Label>
                <Input
                  name="next_service_mileage"
                  type="number"
                  defaultValue={editMaint?.next_service_mileage ?? ""}
                />
              </div>
            </div>
            <div>
              ...
              <Input
                name="description"
                defaultValue={editMaint?.description ?? ""}
              />
            </div>
            <div>
              <Label>Notes</Label>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  name="full_name"
                  required
                  defaultValue={editDrv?.full_name ?? ""}
                />
              </div>
              <div>
                <Label>Phone</Label>
                <Input name="phone" defaultValue={editDrv?.phone ?? ""} />
              </div>
              <div>
                <Label>Email</Label>
                <Input
                  name="email"
                  type="email"
                  defaultValue={editDrv?.email ?? ""}
                />
              </div>
              <div>
                <Label>Hub</Label>
                <Select name="hub" defaultValue={editDrv?.hub ?? "phoenix"}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
              ...
              <div>
                ...
                <Select
                  name="status"
                  defaultValue={editDrv?.status ?? "active"}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  type="number"
                  step="0.01"
                  defaultValue={editDrv?.hourly_rate ?? ""}
                />
              </div>
              <div>
                <Label>License #</Label>
                <Input
                  name="license_number"
                  defaultValue={editDrv?.license_number ?? ""}
                />
              </div>
              <div>
                ...
                <Input
                  name="license_expiry"
                  type="date"
                  defaultValue={editDrv?.license_expiry ?? ""}
                />
              </div>
              <div>
                ...
                <Input
                  name="hired_date"
                  type="date"
                  defaultValue={editDrv?.hired_date ?? ""}
                />
              </div>
            </div>
            <div>
                ...
                <div>
                  ...
                  <div className="space-y-1">
                    {odometerHistory.map((o, i) => {
                      ...
                      const diff = prev
                        ? o.odometer_reading - prev.odometer_reading
                        : null;
                      return (
                        <div
                          key={i}
                        ...
                        >
                          <div>
                            <span className="font-mono font-semibold">
                              {o.odometer_reading.toLocaleString()} mi
                            </span>
                            {o.driver_name && (
                          ...
                          <div className="flex items-center gap-2">
                            {diff !== null && (
                              <span
                                className={`text-xs flex items-center gap-0.5 ${diff >= 0 ? "text-green-500" : "text-red-500"}`}
                              >
                                {diff > 0 ? (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <Gauge className="h-3 w-3" />{" "}
                            {insp.odometer_reading.toLocaleString()} mi
                          </span>
                          {insp.checklist.fuel_level && (
                            <span>Fuel: {insp.checklist.fuel_level}</span>
                          )}
                        </div>
                        <div className="flex flex-wrap gap-1">
                          {(() => {
                            const cl = insp.checklist as Record<
                              string,
                              unknown
                            >;
                            const checks: {
                            ...
                            return checks.map(({ key, label, invertBad }) => {
                              ...
                              const isGood = invertBad
                                ? norm.result === "issue"
                                : norm.result === "pass";
                              const isBad = invertBad
                                ? norm.result === "pass"
                                : norm.result === "issue";
                              if (isGood)
                                return (
                                  <Badge
                                    key={key}
                                    variant="secondary"
                                    className="text-[9px] bg-green-500/10 text-green-700 dark:text-green-400"
                                  >
                                    {label} OK
                                    {norm.note ? ` (${norm.note})` : ""}
                                  </Badge>
                              ...
                              if (isBad)
                                return (
                                  <Badge
                                    key={key}
                                    variant="secondary"
                                    className="text-[9px] bg-orange-500/10 text-orange-700 dark:text-orange-400"
                                  >
                                    {label} - Issue
                                    {norm.note ? ` (${norm.note})` : ""}
                                  </Badge>
              ...
              {inspectionHistory.length === 0 &&
                odometerHistory.length === 0 && (
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p className="text-xs">
                      No walk-around inspections in the last 30 days.
                    </p>
                    <Button
                    ...
                    >
                      <ClipboardCheck className="h-3.5 w-3.5" /> Start First
                      Inspection
                    </Button>
                  </div>
                )}
      ...
      {deleteTarget &&
        (() => {
          const hasMaintenanceRecords =
            deleteTarget.type === "vehicle" &&
            maintenance.some((m) => m.vehicle_id === deleteTarget.id);
          const maintCount =
            deleteTarget.type === "vehicle"
              ? maintenance.filter((m) => m.vehicle_id === deleteTarget.id)
                  .length
              : 0;
          return (
            <Dialog
              ...
              <DialogContent>
                ...
                {hasMaintenanceRecords && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-red-500/10 border border-red-500/20 text-red-600 text-sm">
                    ...
                    <span>
                      {maintCount} maintenance record{maintCount > 1 ? "s" : ""}{" "}
                      will be permanently deleted with this vehicle.
                    </span>
                  </div>
                )}
                <DialogFooter>
