...
function computeAge(alert: RouteAlert): number {
  return Math.round(
    (Date.now() - new Date(alert.created_at).getTime()) / 60_000,
  );
}
...
export function useAlerts(options: UseAlertsOptions = {}) {
  ...
  useEffect(() => {
    alerts.forEach((alert) => {
      if (
        alert.escalatedSeverity === "auto_ping" &&
        !autoPingedRef.current.has(alert.id)
      ) {
  ...
  const stats = useMemo(
    () => ({
      ...
      critical: alerts.filter(
        (a) =>
          a.escalatedSeverity === "critical" ||
          a.escalatedSeverity === "auto_ping",
      ).length,
