...
export function useDispatchBlast() {
  ...
  const fetchBlasts = useCallback(async () => {
    const { data: blastRows, error } = (await supabase
      ...
      .select("*")
      .or(
        "status.eq.active,created_at.gte." +
    ...
    const { data: responseRows } = (await supabase
      ...
      .order("notified_at", { ascending: true })) as {
      data: BlastResponse[] | null;
    };
    ...
    async (params: CreateBlastParams): Promise<DispatchBlast | null> => {
      ...
      try {
        ...
        const loadBody = loadData
          ...
          : (params.message ??
            "New load available -- open the app to view details.");
    ...
    async (blastId: string, driverId: string, lat?: number, lng?: number) => {
      ...
      if (blast && isBlastExpired(blast)) {
        toast({
          title: "Blast Expired",
          description:
            "This blast has expired and is no longer accepting responses.",
          variant: "destructive",
        });
        return false;
      }
    ...
    async (blastId: string, driverId: string) => {
      ...
      if (blast && isBlastExpired(blast)) {
        toast({
          title: "Blast Expired",
          description:
            "This blast has expired. Create a new blast to assign this load.",
          variant: "destructive",
        });
        return false;
      }
      ...
      const { data, error } = (await supabase.rpc("confirm_blast_assignment", {
        ...
        data: { success: boolean; error?: string; load_id?: string } | null;
        error: any;
      };
      ...
      try {
        await sendPushToDrivers(
          ...
          {
            load_id: data.load_id ?? blastId,
            blast_id: blastId,
            type: "blast_confirmed",
          },
        );
      } catch (pushErr) {
        console.warn(
          "[useDispatchBlast] Push to assigned driver failed:",
          pushErr,
        );
      }
        ...
        await supabase
          ...
          .catch(() => {
            ...
            supabase
              ...
              .update({
                drivers_declined:
                  (blasts.find((b) => b.id === blastId)?.drivers_declined ??
                    0) + 1,
              })
              .eq("id", blastId);
          });
  ...
  const analytics = useMemo(() => {
    ...
    const interestedResponses = allResponses.filter(
      (r) => r.status === "interested",
    );
    const avgResponseTime = interestedResponses.length
      ? Math.round(
          interestedResponses.reduce(
            (s, r) => s + (r.response_time_ms ?? 0),
            0,
