...
export function useDispatchData(dateRangeStart: string, dateRangeEnd: string) {
  ...
  const { data: vehicles = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("vehicles")
        .select("id, vehicle_name, vehicle_type, hub, status")
        .eq("status", "active");
      if (error) throw error;
  ...
  const { data: recentAddresses = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("daily_loads")
        .select(
          "pickup_address, delivery_address, pickup_company, delivery_company",
        )
