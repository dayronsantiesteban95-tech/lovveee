  ...
  while (foundImprovement && iterations < maxIterations) {
    ...
    for (let i = 1; i < improved.length - 1; i++) {
      for (let j = i + 1; j < improved.length; j++) {
        const d1 =
          distMatrix[improved[i - 1]][improved[i]] +
          distMatrix[improved[j]][improved[(j + 1) % improved.length]];
        const d2 =
          distMatrix[improved[i - 1]][improved[j]] +
          distMatrix[improved[i]][improved[(j + 1) % improved.length]];
        if (d2 < d1 - 0.01) {
...
export function optimizeRoute(
      ...
      stops: [
        {
          ...
          estimatedArrival: options?.startTime ?? "08:00",
          estimatedDepartMinutes: options?.minutesPerStop ?? 10,
        },
      ],
      totalDistanceMiles: 0,
  ...
  const totalDuration =
    currentMinutes - (startTimeParts[0] * 60 + startTimeParts[1]);

  return {
    stops,
    totalDistanceMiles: Math.round(optimizedDist * 10) / 10,
    totalDurationMinutes: Math.round(totalDuration),
    savingsVsOriginalMiles:
      Math.round((originalDist - optimizedDist) * 10) / 10,
  };
}
