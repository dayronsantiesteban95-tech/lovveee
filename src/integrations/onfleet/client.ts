...
export async function pushLoadToOnfleet(
  payload: PushLoadPayload,
): Promise<OnfleetTask> {
...
export async function updateOnfleetTask(
  ...
  if (updates.completeBefore !== undefined)
    params.completeBefore = updates.completeBefore;
...
export async function syncTaskToLoad(
  ...
  if (task.completionDetails) {
    ...
    if (startEvt)
      payload.start_time = new Date(startEvt.time)
        .toISOString()
        .split("T")[1]
        .slice(0, 5);
    if (endEvt)
      payload.end_time = new Date(endEvt.time)
        .toISOString()
        .split("T")[1]
        .slice(0, 5);
  ...
  if (error)
    throw new Error(`Sync failed for ${task.shortId}: ${error.message}`);
  return payload;
}
