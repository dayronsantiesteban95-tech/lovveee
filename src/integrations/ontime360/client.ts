...
export async function getTodayOrders(): Promise<OT360Order[]> {
  ...
  return callEdge<OT360Order[]>("listOrders", {
    dateFrom: today,
    dateTo: today,
  });
}
...
export async function pushLoadToOnTime360(
...
export async function cancelOT360Order(
  orderId: string,
  reason?: string,
): Promise<void> {
...
export async function syncOrderToLoad(
  ...
  if (
    ...
    (order.proofOfDeliveryPhotos && order.proofOfDeliveryPhotos.length > 0)
  ) {
    payload.pod_confirmed = true;
  }
  if (order.dateCompleted) {
    payload.end_time = new Date(order.dateCompleted)
      .toISOString()
      .split("T")[1]
      .slice(0, 5);
  }
  ...
  if (error)
    throw new Error(
      `OT360 sync failed for ${order.trackingNumber}: ${error.message}`,
    );
  return payload;
}
...
export async function getLiveGPS(
  ...
  return drivers
    .filter(
      (d) => d.status === "Active" && d.currentLatitude && d.currentLongitude,
    )
    .map((d) => ({
