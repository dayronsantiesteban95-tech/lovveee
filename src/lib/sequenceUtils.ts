 ...
 */
async function getCadenceSettings(): Promise<{
  ...
  for (const row of data) {
    if (row.setting_key === "email1_to_email2_days")
      result.email1_to_email2_days =
        parseInt(row.setting_value) || DEFAULT_CADENCE.email1_to_email2_days;
    if (row.setting_key === "email2_to_call_days")
      result.email2_to_call_days =
        parseInt(row.setting_value) || DEFAULT_CADENCE.email2_to_call_days;
  }
  return result;
}
 ...
 */
export async function createSequenceForLead(
  leadId: string,
  userId: string,
): Promise<boolean> {
  ...
  const rows = [
    ...
    },
    {
      ...
      step_type: "email_2",
      status: "pending",
      follow_up_date: format(addDays(new Date(), e2Days), "yyyy-MM-dd"),
      response_status: "no_response",
      created_by: userId,
    },
    {
      ...
      step_type: "call",
      status: "pending",
      follow_up_date: format(addDays(new Date(), callDays), "yyyy-MM-dd"),
      response_status: "no_response",
      created_by: userId,
    },
  ];
