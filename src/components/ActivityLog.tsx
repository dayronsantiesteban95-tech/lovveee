...
function getActionMeta(action: string, entityType: string) {
  if (action.includes("created") || action.includes("new"))
    return { icon: Package, color: "text-green-500", bg: "bg-green-500/10" };
  ...
  if (action.includes("picked") || action.includes("transit"))
    return { icon: MapPin, color: "text-amber-500", bg: "bg-amber-500/10" };
  if (action.includes("pod") || action.includes("photo"))
    return { icon: Camera, color: "text-orange-500", bg: "bg-orange-500/10" };
  if (action.includes("route") || action.includes("optimize"))
    return { icon: Route, color: "text-violet-500", bg: "bg-violet-500/10" };
  if (action.includes("import"))
    return { icon: FileText, color: "text-indigo-500", bg: "bg-indigo-500/10" };
...
export default function ActivityLog({
  ...
  const fetchActivity = useCallback(async () => {
    ...
    try {
      let query = supabase
        ...
        .select(
          "id, load_id, new_status, previous_status, reason, created_at, changed_by",
        )
    ...
    try {
      let query = supabase
        .from("daily_loads")
        .select(
          "id, reference_number, client_name, status, created_at, driver_id, updated_at",
        )
        .order("created_at", { ascending: false })
        .limit(limit);
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardContent
        className={`${compact ? "pt-3 pb-2" : "pt-4 pb-3"} space-y-3`}
      >
        ...
        {loading ? (
          ...
          <div
            className={`space-y-0.5 ${compact ? "max-h-48" : "max-h-96"} overflow-y-auto`}
          >
            {filtered.map((entry, i) => {
              ...
              return (
                ...
                >
                  <div
                    className={`h-6 w-6 rounded-full ${meta.bg} flex items-center justify-center shrink-0 mt-0.5`}
                  >
