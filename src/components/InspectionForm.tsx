...
export default function InspectionForm({
    ...
    async (value: string) => {
      ...
      if (data && data.length > 0) {
        ...
        if (num < lastReading) {
          ...
          setOdometerWarning(
            `Note: Same as last reading (${lastReading.toLocaleString()} mi). Is this correct?`,
          );
        } else {
  ...
  const handlePhotoSelect = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      for (const file of files) {
        ...
        setPhotos((prev) => [
          ...prev,
          { label, url: urlData.publicUrl, path, file, preview },
        ]);
      }
    } finally {
      setUploading(false);
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!checklist.fuel_level) {
      toast({
        title: "Fuel level required",
        description: "Please select the fuel level.",
        variant: "destructive",
      });
      return;
    }
    if (checklist.exterior_damage && !notes.trim()) {
      toast({
        ...
        description:
          "Exterior damage was marked -- please describe it in please describe it in the notes.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      if (insertError) {
        ...
        if (insertError.code === "23505") {
          toast({
            ...
            description:
              "An inspection for this vehicle was already submitted today.",
            variant: "destructive",
          });
        } else {
          toast({
      ...
      if (carWashDone) {
        try {
          ...
          captureScopedError(
            "fleet",
            { vehicleId, driverId, operation: "log_car_wash" },
            washErr,
          );
      ...
      toast({
        ...
        description: "Walk-around inspection recorded successfully.",
      });
      onSuccess?.();
    } catch (err) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="rounded-xl border border-border/60 divide-y divide-border/40">
          {[
            {
              key: "tires_ok" as const,
              label: "Tires OK",
              desc: "No flats, proper inflation",
            },
            {
              key: "lights_ok" as const,
              label: "Lights OK",
              desc: "All exterior lights working",
            },
            {
              key: "brakes_ok" as const,
              label: "Brakes OK",
              desc: "No unusual sounds or feel",
            },
            {
              key: "fluids_ok" as const,
              label: "Fluids OK",
              desc: "Oil, coolant, washer fluid",
            },
            {
              key: "interior_clean" as const,
              label: "Interior Clean",
              desc: "Cab is clean and organized",
            },
            {
              key: "exterior_damage" as const,
              label: "Exterior Damage",
              desc: "Any dents, scratches, or damage",
            },
          ].map(({ key, label, desc }) => (
            <div
              key={key}
        ...
        {checklist.exterior_damage && (
          <div className="flex items-center gap-2 text-xs text-orange-600 bg-orange-500/10 rounded-lg px-3 py-2">
            ...
            Exterior damage noted ??" please describe it in the Notes section
            below.
          </div>
        )}
      </div>
        ...
        {photos.length > 0 && (
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {photos.map((p, i) => (
              <div
                key={i}
                className="relative group rounded-lg overflow-hidden border border-border/60 aspect-square"
              >
                <img
                  src={p.preview}
                  alt={p.label}
      ...
      <div className="flex items-center gap-3 rounded-xl border border-border/60 px-4 py-3">
        ...
        />
        <div>
          <Label htmlFor="car_wash_done" className="cursor-pointer">
            ???? Car wash done today?
          </Label>
          <p className="text-xs text-muted-foreground">
            This will log a car wash and reset the 10-day timer
          </p>
        </div>
      </div>
