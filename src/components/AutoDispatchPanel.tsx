...
function confidenceLevel(top: DriverCandidate): "HIGH" | "MEDIUM" | "LOW" {
  if (
    top.shiftStatus === "on_duty" &&
    top.distanceMiles < 10 &&
    top.activeLoadCount === 0
  ) {
    return "HIGH";
  }
...
export default function AutoDispatchPanel({
  ...
  >({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("daily_loads")
        .select(
          "id, reference_number, client_name, pickup_address, pickup_lat, pickup_lng, status",
        )
  ...
  const suggestions: LoadSuggestion[] = loads.map((load) => {
    ...
    const confidence =
      candidates.length > 0 ? confidenceLevel(candidates[0]) : "LOW";
  ...
  return (
    <Card className="border-0 shadow-sm">
      <CardContent className="pt-4 pb-3 space-y-3">
        ...
        {/* Suggestion cards */}
        {!loadsLoading &&
          suggestions.map(({ load, candidates, confidence }) => (
            <div
              key={load.id}
