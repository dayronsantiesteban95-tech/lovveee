...
const PAGES: SearchResult[] = [
  ...
  },
  {
    ...
    subtitle: "Load board, live ops, reports",
    icon: Package,
...
function statusBadge(status: string): { badge: string; badgeColor: string } {
  switch (status) {
    ...
    case "in_progress":
      return {
        badge: "In Transit",
        badgeColor: "bg-yellow-500/15 text-yellow-600",
      };
    case "picked_up":
      return { badge: "Picked Up", badgeColor: "bg-blue-500/15 text-blue-600" };
    case "assigned":
      return {
        badge: "Assigned",
    ...
    case "cancelled":
      return { badge: "? Cancelled", badgeColor: "bg-red-500/15 text-red-600" };
    default:
      return { badge: status, badgeColor: "bg-slate-500/15 text-slate-600" };
  }
}
    ...
    async (q: string) => {
      ...
      if (
        ...
        allResults.push({
          ...
          subtitle: "Add a delivery to the load board",
          icon: Plus,
          iconColor: "text-green-500",
          action: () => {
            navigate("/dispatch");
      ...
      if (
        ...
        allResults.push({
          id: "action-import",
          category: "actions",
          title: "Import Loads from CSV",
          subtitle: "Bulk upload from spreadsheet",
          icon: FileText,
          iconColor: "text-orange-500",
          action: () => {
            navigate("/dispatch");
      ...
      if (term.includes("route") || term.includes("optimize")) {
        allResults.push({
          ...
          subtitle: "Auto-arrange delivery order",
          icon: Route,
          iconColor: "text-violet-500",
          action: () => {
            navigate("/dispatch");
      ...
      try {
        const { data: loads } = (await supabase
          .from("daily_loads")
          .select(
            "id, reference_number, client_name, delivery_address, status, tracking_token, load_date",
          )
          .or(
            `reference_number.ilike.%${term}%,client_name.ilike.%${term}%,delivery_address.ilike.%${term}%,tracking_token.ilike.%${term}%`,
          )
      ...
      try {
        const { data: drivers } = (await supabase
          ...
          .or(
            `full_name.ilike.%${term}%,hub.ilike.%${term}%,phone.ilike.%${term}%`,
          )
        ...
        if (contacts) {
          for (const c of contacts) {
            allResults.push({
              ...
              title: c.name,
              subtitle:
                `${c.company ?? ""} ${c.email ? "| " + c.email : ""}`.trim(),
              icon: Users,
              iconColor: "text-pink-500",
              action: () => {
                navigate("/contacts");
        ...
        <div className="bg-background rounded-xl border shadow-2xl overflow-hidden">
            ...
            {query && grouped.length === 0 && !searching && (
              <div className="px-4 py-8 text-center">
                ...
                <p className="text-xs text-muted-foreground/50 mt-1">
                  Try searching by name, reference #, or address
                </p>
              </div>
            )}
              ...
              <div key={group.category} className="px-3 py-1">
                ...
                {group.items.map((result) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`h-8 w-8 rounded-lg bg-muted/50 flex items-center justify-center shrink-0`}
                      >
                      ...
                      {result.badge && (
                        <span
                          className={`text-[9px] px-1.5 py-0.5 rounded-full ${result.badgeColor ?? "bg-muted text-muted-foreground"} whitespace-nowrap shrink-0`}
                        >
                          {result.badge}
                        </span>
                      )}
          ...
          <div className="px-4 py-2 border-t bg-muted/20 flex items-center gap-4 text-[10px] text-muted-foreground/50">
            <span className="flex items-center gap-1">
              <kbd className="px-1 rounded bg-muted border font-mono">
                Cmd+K
              </kbd>{" "}
              Navigate
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1 rounded bg-muted border font-mono">?</kbd>{" "}
              Select
            </span>
            <span className="flex items-center gap-1">
              <kbd className="px-1 rounded bg-muted border font-mono">ESC</kbd>{" "}
              Close
            </span>
