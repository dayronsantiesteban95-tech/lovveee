...
export default function RouteOptimizerPanel({
  ...
  const handleOptimize = async () => {
    ...
    if (points.length < 2) {
      toast({
        ...
        description:
          "At least 2 deliverable loads with addresses are required.",
      });
      setLoading(false);
      return;
    }
    ...
    toast({
      ...
      description: `${
        result.savingsVsOriginalMiles > 0
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      captureLoadError(optimized.stops[0]?.id || "unknown", err, {
        operation: "apply_route_order",
      });
    } finally {
      setApplying(false);
    }
      ...
      <CardContent className="pt-4 pb-3 space-y-3">
        ...
        {driverName && (
          <p className="text-xs text-muted-foreground flex items-center gap-1">
            <Truck className="h-3 w-3" /> {driverName} ? Shift starts{" "}
            {shiftStart}
          </p>
        )}
        ...
        >
          {loading ? (
            <>
              <RefreshCw className="h-4 w-4 animate-spin" /> Geocoding &
              optimizing...
            </>
          ) : optimized ? (
            <>
          ...
          <div className="space-y-3 mt-2">
            ...
            <div className="grid grid-cols-3 gap-2 text-center">
              ...
              <div className="p-2 rounded-lg bg-muted/30">
                ...
                <p className="text-sm font-bold">
                  {Math.floor(optimized.totalDurationMinutes / 60)}h{" "}
                  {optimized.totalDurationMinutes % 60}m
                </p>
              </div>
              <div
                className={`p-2 rounded-lg ${optimized.savingsVsOriginalMiles > 0 ? "bg-green-500/10" : "bg-muted/30"}`}
              >
                <p className="text-xs text-muted-foreground">Saved</p>
                <p
                  className={`text-sm font-bold ${optimized.savingsVsOriginalMiles > 0 ? "text-green-600" : ""}`}
                >
                  {optimized.savingsVsOriginalMiles > 0 ? (
                    <span className="flex items-center justify-center gap-0.5">
                      <TrendingDown className="h-3 w-3" />{" "}
                      {optimized.savingsVsOriginalMiles} mi
                    </span>
                  ) : (
                    "--"
                  )}
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        {load?.tracking_token && (
                          <Button
                            ...
                            onClick={() =>
                              copyTrackingLink(load.tracking_token)
                            }
                          >
