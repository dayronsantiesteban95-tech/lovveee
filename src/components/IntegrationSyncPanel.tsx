    ...
    async (source: SyncSource) => {
      ...
      try {
        ...
        if (!handler) {
          toast({
            title: "Not connected",
            description: `${source} integration is not configured.`,
            variant: "destructive",
          });
          return;
        }
    ...
    <Card className="border-0 shadow-sm">
      <CardContent className="pt-4 pb-3 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {/* Onfleet */}
          <div
            className={`rounded-xl border p-3 transition-all ${onfleetConnected ? "border-green-500/30 bg-green-500/5" : "border-dashed border-muted-foreground/20 bg-muted/5"}`}
          >
          ...
          {/* OnTime 360 */}
          <div
            className={`rounded-xl border p-3 transition-all ${ontime360Connected ? "border-blue-500/30 bg-blue-500/5" : "border-dashed border-muted-foreground/20 bg-muted/5"}`}
          >
            <div className="flex items-center gap-2 mb-2">
              {ontime360Connected ? (
        ...
        {results && (
          <div className="rounded-lg bg-muted/30 px-3 py-2 text-[11px] flex items-center gap-2">
            ...
            <span>
              ...
              <strong>
                {results.source === "onfleet" ? "Onfleet" : "OnTime 360"}
              </strong>
              {results.errors.length > 0 && (
