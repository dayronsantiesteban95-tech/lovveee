...
const PRIORITY_CONFIG: Record<
  BlastPriority,
  { label: string; color: string; icon: string; defaultMinutes: number }
> = {
  low: {
    label: "Low",
...
const DispatchBlastPanel = memo(function DispatchBlastPanel({
  ...
  const eligibleLoads = useMemo(
    () =>
      loads.filter(
        (l) =>
          ["assigned", "pending", "unassigned"].includes(l.status) ||
          l.id === selectedLoad,
      ),
    [loads, selectedLoad],
  );
  ...
  const hubDrivers = useMemo(() => {
    ...
    return drivers.filter(
      (d) =>
        d.status === "active" && (d.hub === currentLoad.hub || d.hub === "any"),
    );
  }, [drivers, currentLoad]);
  ...
  const selectAll = () => {
    setSelectedDrivers(
      new Set([...hubDrivers, ...otherDrivers].map((d) => d.id)),
    );
  };
          ...
          <CardContent className="pt-4 pb-3 space-y-3">
              ...
              <div className="rounded-lg bg-muted/30 border border-border/30 p-2.5 text-xs space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-semibold">
                    {currentLoad.client_name ?? "Unknown Client"}
                  </span>
                  <Badge variant="outline" className="text-[9px]">
                    {currentLoad.service_type}
                  </Badge>
                </div>
                <div className="flex items-center gap-1 text-muted-foreground">
                  <MapPin className="h-3 w-3" />
                  {currentLoad.pickup_address ?? "--"} &rarr;{" "}
                  {currentLoad.delivery_address ?? "--"}
                </div>
                ...
                {loadBlasts.length > 0 && (
                  <div className="flex items-center gap-1 mt-1">
                    ...
                    <span className="text-yellow-600 text-[10px]">
                      Already has {loadBlasts.length} active blast
                      {loadBlasts.length > 1 ? "s" : ""}
                    </span>
                  </div>
                )}
              </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                ...
                <Select
                  value={priority}
                  onValueChange={(v) => setPriority(v as BlastPriority)}
                >
                  <SelectTrigger className="h-8 text-xs">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {(
                      Object.entries(PRIORITY_CONFIG) as [
                        BlastPriority,
                        (typeof PRIORITY_CONFIG)[BlastPriority],
                      ][]
                    ).map(([key, cfg]) => (
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  <Input
                    ...
                    value={expiryMinutes}
                    onChange={(e) =>
                      setExpiryMinutes(Number(e.target.value) || 30)
                    }
            ...
            >
              ...
              <span className="relative flex items-center gap-2">
                {sending ? (
                  ...
                  <>
                    <Signal className="h-4 w-4" /> Blast to{" "}
                    {selectedDrivers.size} Driver
                    {selectedDrivers.size > 1 ? "s" : ""}
                  </>
                )}
              </span>
            </Button>
        ...
        {!loading && blasts.length === 0 && !showCreate && (
          <div className="text-xs text-muted-foreground text-center py-6 space-y-1">
            ...
            <p className="text-[10px]">
              Select a load and blast it to your drivers!
            </p>
          </div>
        )}
...
function BlastCard({
  ...
  const priCfg =
    PRIORITY_CONFIG[blast.priority as BlastPriority] ?? PRIORITY_CONFIG.normal;
  ...
  const pending = blast.responses.filter(
  ...
  const elapsed = Math.round(
    (Date.now() - new Date(blast.blast_sent_at).getTime()) / 1000,
  );
  const elapsedStr =
    elapsed < 60
      ? `${elapsed}s ago`
  ...
  const remaining = blast.expires_at
    ? Math.max(
        0,
        Math.round((new Date(blast.expires_at).getTime() - Date.now()) / 1000),
      )
    : null;
  const remainingStr =
    remaining !== null
    ...
    >
      ...
      >
        ...
        <div className="relative">
          ...
          <Radio
            className={`h-4 w-4 ${isActive ? "text-primary" : isAccepted ? "text-green-500" : "text-muted-foreground"}`}
          />
        </div>
        ...
        <div className="flex-1 text-left">
          <div className="flex items-center gap-1.5">
            ...
            <Badge
              variant="outline"
              className={`text-[9px] px-1 py-0 ${priCfg.color}`}
            >
              {priCfg.label}
            </Badge>
        ...
        <div className="border-t border-border/30 px-3 py-2 space-y-2">
          ...
          <div className="space-y-1">
            ...
            {blast.responses.map((r) => {
              const driverName =
                drivers.find((d) => d.id === r.driver_id)?.full_name ??
                "Unknown";
              ...
              return (
                ...
                >
                  ...
                  {r.status === "pending" && (
                    <Clock className="h-3.5 w-3.5 text-muted-foreground shrink-0 animate-pulse" />
                  )}
                  {r.status === "expired" && (
                    <Ban className="h-3.5 w-3.5 text-muted-foreground/50 shrink-0" />
                  )}

                  <span
                    className={`truncate ${r.status === "interested" ? "font-semibold text-green-700" : ""}`}
                  >
                    {driverName}
                  </span>
                  ...
                  {r.decline_reason && (
                    <span
                      className="text-[9px] text-red-400 truncate max-w-[80px]"
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-[9px] text-muted-foreground">
                <span>
                  ...
                  {blast.drivers_notified} responded
                </span>
              ...
              <div className="h-1.5 rounded-full bg-muted/30 overflow-hidden">
                <div className="h-full flex">
                  <div
                    ...
                    style={{
                      width: `${(interested.length / blast.drivers_notified) * 100}%`,
                    }}
                  />
                  <div
                    className="bg-red-400 transition-all"
                    style={{
                      width: `${(declined.length / blast.drivers_notified) * 100}%`,
                    }}
                  />
                  <div
                    className="bg-blue-400 transition-all"
                    style={{
                      width: `${(viewed.length / blast.drivers_notified) * 100}%`,
                    }}
                  />
                </div>
              </div>
