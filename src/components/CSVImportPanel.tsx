    ...
    (file: File) => {
      ...
      reader.onload = (e) => {
        ...
        if (headers.length === 0) {
          toast({
            title: "Empty file",
            description: "No data found in the CSV.",
            variant: "destructive",
          });
          return;
        }
  ...
  const handleImport = async () => {
    ...
    for (const p of payloads) {
      ...
      if (pickup && !(p as any).pickup_lat) {
        ...
        if (coords) {
          (p as any).pickup_lat = coords.lat;
          (p as any).pickup_lng = coords.lng;
        }
      }
      if (delivery && !(p as any).delivery_lat) {
        const coords = await geocodeAddress(delivery);
        if (coords) {
          (p as any).delivery_lat = coords.lat;
          (p as any).delivery_lng = coords.lng;
        }
      }
    }
    ...
    for (let i = 0; i < payloads.length; i += BATCH_SIZE) {
      ...
      try {
        ...
        captureScopedError(
          "csv_import",
          { batchNumber: i / BATCH_SIZE + 1, batchSize: batch.length },
          err,
        );
      }
    }
    ...
    <Card className="border-0 shadow-sm">
      <CardContent className="pt-4 pb-3 space-y-3">
        ...
        {step === "upload" && (
          ...
          >
            <Upload
              className={`h-8 w-8 mx-auto mb-2 ${dragOver ? "text-primary" : "text-muted-foreground/40"}`}
            />
                  ...
                  <thead>
                    <tr>
                      {Object.entries(columnMap).map(([csvIdx, dbCol]) => (
                        <th
                          key={csvIdx}
                          className="text-left p-1 border-b font-semibold"
                        >
                    ...
                    {csvRows.slice(0, 3).map((row, i) => (
                      <tr key={i}>
                        {Object.keys(columnMap).map((csvIdx) => (
                          <td
                            key={csvIdx}
                            className="p-1 border-b truncate max-w-[120px]"
                          >
                            {row[Number(csvIdx)] ?? "--"}
                          </td>
                        ))}
