...
const BlastStatusPanel = memo(function BlastStatusPanel({
  blastId,
  onCancelled,
  onAssigned,
  compact = false,
  ...
  const fetchBlastData = useCallback(async () => {
    ...
    const { data: blastData, error: blastErr } = await supabase
      ...
      .select(
        "id, load_id, status, drivers_notified, expires_at, blast_sent_at, accepted_by, message",
      )
      .eq("id", blastId)
      .single();
    ...
    const { data: responseData } = await supabase
      ...
      .select(
        "id, blast_id, driver_id, status, responded_at, notified_at, drivers(full_name, hub)",
      )
      .eq("blast_id", blastId)
        ...
        (payload) => {
          ...
          if (updated.status === "accepted" && updated.accepted_by) {
            ...
            toast({
              title: "? Load Assigned!",
              description:
                "A driver accepted the blast -- load has been assigned.",
            });
          } else if (updated.status === "expired") {
            toast({
              title: "? Blast Expired",
              description:
                "No driver accepted in time. Load is back to unassigned.",
              variant: "destructive",
            });
          }
        },
            ...
            {isActive && timeLeft !== null && (
              <div
                className={`flex items-center gap-1 text-xs font-mono ${
                  timeLeft < 120 ? "text-red-500" : "text-muted-foreground"
                }`}
              >
          ...
          <div className="space-y-1">
              ...
              <span>
                {Math.round(
                  ((accepted.length + declined.length) /
                    Math.max(blast.drivers_notified, 1)) *
                    100,
                )}
                % responded
              </span>
            ...
            <div className="h-1.5 rounded-full bg-muted/30 overflow-hidden flex">
              <div
                ...
                style={{
                  width: `${(accepted.length / Math.max(blast.drivers_notified, 1)) * 100}%`,
                }}
              />
              <div
                className="bg-red-400 transition-all duration-500"
                style={{
                  width: `${(declined.length / Math.max(blast.drivers_notified, 1)) * 100}%`,
                }}
              />
            </div>
          </div>
...
export default BlastStatusPanel;
...
function StatusBadge({ status }: { status: string }) {
  const configs: Record<string, { label: string; className: string }> = {
    active: {
      label: "LIVE",
      className:
        "bg-green-500/15 text-green-600 border-green-500/25 animate-pulse",
    },
    accepted: {
      label: "Accepted",
      className: "bg-green-500/15 text-green-600 border-green-500/25",
    },
    expired: {
      label: "Expired",
  ...
  return (
    <Badge
      variant="outline"
      className={`text-[9px] px-1.5 py-0 ${cfg.className}`}
    >
      {cfg.label}
    </Badge>
  );
